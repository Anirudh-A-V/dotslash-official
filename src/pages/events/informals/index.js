...
import { getInformals, urlFor } from '@/utils/sanity';
...
const InformalsPage = () => {
    ...
    useEffect(() => {
        const getInformalsData = async () => {
            const Informals = await getInformals()
            setPreEventData(Informals)
            setLoading(false)
        }
        getInformalsData()
    ...
    return (
        <>
            ...
            >
                <Navbar />
                <h1 className={`text-white text-4xl text-center font-medium mt-20 mb-8 ${PPNeueMontrealFont.className}`}>PRE-EVENTS</h1>
                {
                    loading ? (
